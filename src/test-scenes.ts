...
export async function createSpotScene(): Promise<wt.Scene> {
    ...
    let spottexture: wt.Texture | undefined = await wt.TextureLoader.load("assets/spot_texture.png", true); //flip true

    let objMaterial: wt.Material = new wt.Material(spottexture);
    objMaterial.roughness = 0.9;
    objMaterial.metallic = 0.1;

    let objModel: wt.MeshModel = new wt.MeshModel(objMesh, objMaterial);
    objModel.position = new THREE.Vector3(0, 1, 0);
    objModel.euler = new THREE.Euler(0, degToRad(180), 0);
    
    objModel.scale = new THREE.Vector3(1.5, 1.5, 1.5);
    scene.add(objModel);
